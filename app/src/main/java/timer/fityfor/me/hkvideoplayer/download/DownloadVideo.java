...
import timer.fityfor.me.hkvideoplayer.utils.LogUtils;
...
public class DownloadVideo extends AsyncTask<String, String, String> {
    ...
    protected void onPostExecute(String file_url) {
        if (!isExists) {
            ViewController.getViewController().showToast("Video was successfully downloaded");
        }
    }

    private void downloadFile(String fileName) {
        String rootDir = Environment.getExternalStorageDirectory().getAbsolutePath();
        File rootFile = new File(rootDir,"Videos");
        if (!rootFile.exists()){
            rootFile.mkdirs();
        }

        if (isExists(rootDir + "/Videos/" + fileName)) {
            d("File is exists...");
            isExists = true;
        } else {
            d("File is downloading...");
            ViewController.getViewController().showToast("downloading...");
            isExists = false;
            try {
